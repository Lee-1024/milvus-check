...
func Definitions() []Definition {
	return []Definition{
		...
		def("request_success_rate", "overview", "请求成功率", "成功请求占全部请求的百分比。", "持续低于 99% 时应检查错误日志。", "Proxy", "%", "line", "milvus_proxy_req_count", `100 * sum(rate(milvus_proxy_req_count{$match,status="success"}[$rate])) / clamp_min(sum(rate(milvus_proxy_req_count{$match,status="total"}[$rate])), 1e-9)`),
		...
		def("load_success_rate", "load_index", "集合加载成功率", "QueryCoord 集合加载请求的成功比例。", "失败时结合集合加载表和 QueryCoord 日志排查。", "QueryCoord", "%", "line", "milvus_querycoord_load_req_count", `100 * sum(rate(milvus_querycoord_load_req_count{$match,status="success"}[$rate])) / clamp_min(sum(rate(milvus_querycoord_load_req_count{$match,status="total"}[$rate])), 1e-9)`),
